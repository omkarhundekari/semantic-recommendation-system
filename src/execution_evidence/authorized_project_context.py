...
class AuthorizedProjectContext(BaseModel):
    """Request-scoped proof of project tenancy access.

    This context proves only that the authenticated
    principal has active workspace membership for the
    selected project. It does not grant operation-specific
    permissions.
    """
    ...
    def preserve_exact_scope(
        ...
        if value != value.strip():
            raise ValueError(
                "Authorized project scope values must "
