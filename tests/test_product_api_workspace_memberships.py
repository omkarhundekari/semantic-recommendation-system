...
def test_manager_can_transition_workspace_membership_status():
    from datetime import datetime, timezone
    ...
    target_membership_id = (
        "wsm_123e4567-e89b-42d3-a456-426614174021"
    )
    transition_id = (
        "wmt_123e4567-e89b-42d3-a456-426614174022"
    )
    ...
    result = WorkspaceMembershipMutationResult(
        membership=membership,
        transition=transition,
    )
    ...
    try:
        with TestClient(app) as client:
            response = client.patch(
                (
                    f"/v1/workspaces/{WORKSPACE_ID}/"
                    f"memberships/{target_membership_id}/status"
                ),
                json={
                    "status": "suspended",
                    "expected_revision": 1,
    ...
    assert call["new_status"] == "suspended"
    assert call["expected_revision"] == 1
    assert (
        call["changed_by_principal_id"]
        == PRINCIPAL_ID
    )
    assert call["reason"] == "temporary suspension"
    assert call["changed_at"].tzinfo is not None


def test_status_mutation_body_cannot_supply_actor():
    class RecordingStore:
        def __init__(self):
            self.calls = []
    ...
    try:
        with TestClient(app) as client:
            response = client.patch(
                (
                    f"/v1/workspaces/{WORKSPACE_ID}/"
                    f"memberships/{MEMBERSHIP_ID}/status"
                ),
                json={
                    "status": "suspended",
                    "expected_revision": 0,
...
def test_member_without_status_manage_cannot_mutate_status():
    class RecordingStore:
        def __init__(self):
            self.calls = []
...
def test_status_mutation_missing_membership_is_404():
    from execution_evidence.workspace_membership_store import (
        WorkspaceMembershipNotFoundError,
    )
...
def test_status_mutation_revision_conflict_is_409():
    from execution_evidence.workspace_membership_store import (
        WorkspaceMembershipRevisionConflictError,
    )
...
def test_status_mutation_last_manager_is_409():
    from execution_evidence.workspace_membership_store import (
        WorkspaceMembershipLastManagerError,
    )
...
def test_inaccessible_workspace_is_404_before_status_mutation():
    class RecordingStore:
        def __init__(self):
            self.calls = []
        ...
        def transition_status(self, *args, **kwargs):
            self.calls.append((args, kwargs))
            raise AssertionError(
                "Status mutation storage must not run "
                "before workspace tenancy succeeds."
            )
...
def test_status_mutation_transition_conflict_is_409():
    from execution_evidence.workspace_membership_store import (
        WorkspaceMembershipTransitionError,
    )
...
def test_status_mutation_store_failure_is_503():
    from execution_evidence.workspace_membership_store import (
...
def test_status_mutation_forwards_self_removal_to_storage():
    from datetime import datetime, timezone
    ...
    removed_membership = WorkspaceMembership(
        membership_id=MEMBERSHIP_ID,
        workspace_id=WORKSPACE_ID,
        principal_id=PRINCIPAL_ID,
        status="removed",
    ...
    transition = WorkspaceMembershipTransition(
        transition_id=(
            "wmt_123e4567-e89b-42d3-a456-426614174030"
        ),
        membership_id=MEMBERSHIP_ID,
        workspace_id=WORKSPACE_ID,
        principal_id=PRINCIPAL_ID,
    ...
    assert call["membership_id"] == MEMBERSHIP_ID
    assert call["new_status"] == "removed"
    assert call["expected_revision"] == 1
    assert (
        call["changed_by_principal_id"]
        == PRINCIPAL_ID
